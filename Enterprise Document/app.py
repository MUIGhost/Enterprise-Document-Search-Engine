...
DEFAULT_DIRECTORY = os.environ.get("DEFAULT_DIRECTORY", os.path.join(os.path.expanduser("~"), "Documents"))
CORRUPT_FILES_DIR = os.environ.get("CORRUPT_FILES_DIR", os.path.join(tempfile.gettempdir(), "corruptedfiles"))
CACHE_DIR = os.environ.get("CACHE_DIR", os.path.join(tempfile.gettempdir(), "cache"))
...
def libreoffice_to_pdf(file_path, output_path):
    try:
        # Try to find LibreOffice in common locations if not set
        ...
        cmd = [libreoffice_cmd, "--headless", "--convert-to", "pdf", "--outdir", os.path.dirname(output_path), file_path]
...
def preview_file(file_path):
    try:
        last_indexed_dir = session.get('last_indexed_dir', DEFAULT_DIRECTORY)
...
def index_directory_route():
    directory = request.form.get('directory', DEFAULT_DIRECTORY)
...
def index():
    ...
    return render_template("index.html", last_indexed_dir=session.get('last_indexed_dir', DEFAULT_DIRECTORY), public_url=app.config['public_url'])
